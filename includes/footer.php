<footer class="footer">
    <div class="container">
        <div class="footer-content">
            <div class="footer-brand">
                <div class="footer-logo">
                    <span class="material-symbols-rounded brand-icon">fitness_center</span>
                    <span class="brand-text">FitHub</span>
                </div>
                <p class="footer-description">
                    Premium gym management system designed for modern fitness facilities. Transform your gym operations today.
                </p>
            </div>
            
            <div class="footer-links">
                <div class="footer-section">
                    <h4 class="footer-title">System</h4>
                    <ul class="footer-list">
                        <li><a href="/FitHub/index.php">Dashboard</a></li>
                        <li><a href="/FitHub/staff/members.php">Members</a></li>
                        <li><a href="/FitHub/staff/attendance.php">Attendance</a></li>
                        <li><a href="/FitHub/staff/reports.php">Analytics</a></li>
                    </ul>
                </div>
                
                <div class="footer-section">
                    <h4 class="footer-title">Support</h4>
                    <ul class="footer-list">
                        <li><a href="#">Documentation</a></li>
                        <li><a href="#">Help Center</a></li>
                        <li><a href="#">Contact Us</a></li>
                        <li><a href="#">FAQs</a></li>
                    </ul>
                </div>
            </div>
        </div>
        
        <div class="footer-bottom">
            <p>&copy; <?php echo date('Y'); ?> FitHub. All rights reserved. Premium Gym Management System.</p>
        </div>
    </div>
</footer>
